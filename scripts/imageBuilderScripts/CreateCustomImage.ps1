...
'Az.ImageBuilder', 'Az.ManagedServiceIdentity', 'Az.Compute' | ForEach-Object { Install-Module -Name $_ -AllowPrerelease -AllowClobber -Confirm:$false }
...
Write-Host "Identity name:" + $identityName + "get identity:" $identity.Name
...
# Image template name  
$imageTemplateName = "customWinTemplate"
...
New-AzGalleryImageDefinition -GalleryName $galleryName -ResourceGroupName $imageResourceGroup -Location $location -Name $imageDefName -OsState generalized -OsType Windows -Publisher 'myCompany' -Offer 'vscodebox' -Sku $Sku -Feature $features -HyperVGeneration "V2"

# Configure the template with your variables:
$templateFilePath = "../../infra/core/customImageTemplate.json"
